...
extension RawmDefaults {
    static func encoded() -> String? {
        ...
        let config = Config(bundleId: "com.robertarles.rawm",
                            version: version,
                            shortcuts: shortcuts,
    ...
    static func loadFromSupportDir() {
        if let rawmSupportURL = getSupportDir()?
            .appendingPathComponent("rawm", isDirectory: true) {
            
            let configURL = rawmSupportURL.appendingPathComponent("RawmConfig.json")
                        
            let exists = try? configURL.checkResourceIsReachable()
            if exists == true {
                // Defense-in-depth: any process running as this user can drop
                // a RawmConfig.json in Application Support and have it
                ...
                guard isSafe else {
                    AlertUtil.oneButtonAlert(
                        question: "Refused to load RawmConfig.json",
                        text: "The configuration file at \(path) is a symlink or world-writable. rawm has refused to load it. Remove the file or fix its permissions and try again."
                    )
                ...
                let response = AlertUtil.twoButtonAlert(
                    question: "Apply rawm configuration?",
                    text: "A configuration file was found at \(path). Applying it will overwrite your current rawm shortcuts and preferences. Apply now?",
                    confirmText: "Apply",
                    cancelText: "Discard"
                )
                ...
                do {
                    let newFilename = "RawmConfig\(timestamp()).json"
                    
                    try fm.moveItem(atPath: configURL.path, toPath: rawmSupportURL.appendingPathComponent(newFilename).path)
                } catch {
                    do {
                        try fm.removeItem(at: configURL)
                    } catch {
                        AlertUtil.oneButtonAlert(question: "Error after loading from Support Dir", text: "Unable to rename/remove RawmConfig.json from \(rawmSupportURL) after loading.")
                    }
                }
            }
        }
